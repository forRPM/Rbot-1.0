...
async def start_command(message: types.Message):
    await message.answer("Привет! Нажми кнопку, чтобы получить расписание.", reply_markup=build_reply_markup())
...
async def send_schedule(message: types.Message):
    ...
    if not service:
        await message.answer("Произошла ошибка при подключении к OpenAI Drive.")
        return
    files = get_files_from_drive(service, FOLDER_ID)
    if not files:
        await message.answer("В папке нет файлов или произошла ошибка.")
        return

    current_day_number = get_current_date_number()
    schedule_days = [current_day_number, current_day_number + 1, current_day_number + 2]
    ...
    for file in files:
        try:
            ...
            if file_mime_type.startswith('image/'):
              await bot.send_photo(chat_id=message.chat.id, photo=types.BufferedInputFile(file_data, filename=file_name))
            elif file_mime_type == 'application/pdf':
              await bot.send_document(chat_id=message.chat.id, document=types.BufferedInputFile(file_data, filename=file_name))
            else:
              await bot.send_document(chat_id=message.chat.id, document=types.BufferedInputFile(file_data, filename=file_name))

        except HttpError as e:
            logging.error(f"An error occurred while fetching file '{file_name}': {e}")
            await message.answer(f"Не удалось получить файл '{file_name}'.")

    await message.answer("Расписание на сегодня и ближайшие дни отправлено.")
